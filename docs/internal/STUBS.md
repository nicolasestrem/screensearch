...
- **Status**: SUPERSEDED by screensearch-llm
- **Note**: Vision model inference was replaced with embedded Ministral-3B LLM
- **New implementation**: `screensearch-llm/` crate handles local inference via llama.cpp

### Embedded LLM - Download State Tracking
- **Location**: `screensearch-api/src/handlers/ai.rs:456`
- **Status**: TODO - `downloading: false` is hardcoded
- **Description**: Should track actual download progress
- **Current behavior**: Always returns `downloading: false` in model status
- **Impact**: UI cannot show download progress percentage

### Embedded LLM - Server Uptime Tracking
- **Location**: `screensearch-llm/src/server.rs`
- **Status**: Not implemented
- **Description**: Track llama-server process uptime for status endpoint
- **Current behavior**: `uptime_seconds` returns `None`
